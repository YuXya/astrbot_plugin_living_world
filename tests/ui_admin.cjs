...
module.exports = async (page, go, contract) => {
  ...
  await go("chat", "reply"); await field("reply.group_prompt").fill("回复草稿"); await field("social.interjection_interval_minutes").fill("45");
  ...
  await save("保存上下文位置");
  await click("恢复全局初始位置"); await save("保存上下文位置");
