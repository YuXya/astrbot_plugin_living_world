  ...
  try {
    ...
    await page.addInitScript((bootstrap) => {
      ...
      const activities = Array.from({ length: 10 }, (_, i) => {
        ...
        const actions = {};
        for (const [key, marked] of [["news", [1, 5]], ["search", [3, 6]], ["social", [0, 4, 8]]]) actions[key] = { enabled: marked.includes(i), reason: marked.includes(i) ? "活动适合这项行动" : "当前没有具体需要，不凑数", intent: `${key} 活动意图`, at: marked.includes(i) ? start.toISOString() : null, execution: i === 0 && key === "social" ? { status: "skipped", reason: "日程范围外，睡梦中" } : { status: marked.includes(i) ? "pending" : "disabled" } };
