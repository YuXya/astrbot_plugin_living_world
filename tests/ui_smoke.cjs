...
(async () => {
  ...
  try {
    ...
    assert.equal(await firstTarget.locator(".session-details").getAttribute("open"), null, "Technical session details start collapsed");
    await firstTarget.locator(".session-details > summary").focus();
    await page.keyboard.press("Enter");
    await firstTarget.getByText("真实会话：", { exact: false }).waitFor();
    ...
    assert.equal(await firstTarget.locator(".session-details").getAttribute("open"), "", "Inspection keeps expanded details open");
    ...
    await firstTarget.locator(".session-details > summary").click();
    await firstTarget.getByText("对应聊天轮次：round-1", { exact: true }).waitFor();
    if (process.env.LIVING_WORLD_WHITELIST_EXPANDED_SCREENSHOT) await firstTarget.screenshot({ path: process.env.LIVING_WORLD_WHITELIST_EXPANDED_SCREENSHOT });
    await page.setViewportSize({ width: 390, height: 844 });
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth + 1), true, "Expanded session diagnostics wrap on mobile");
    if (process.env.LIVING_WORLD_WHITELIST_EXPANDED_MOBILE_SCREENSHOT) await firstTarget.screenshot({ path: process.env.LIVING_WORLD_WHITELIST_EXPANDED_MOBILE_SCREENSHOT });
    await firstTarget.locator(".session-details > summary").click();
    if (process.env.LIVING_WORLD_WHITELIST_MOBILE_SCREENSHOT) await firstTarget.screenshot({ path: process.env.LIVING_WORLD_WHITELIST_MOBILE_SCREENSHOT });
